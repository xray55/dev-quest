...
<head>
    ...
    <script src="snapshot.js" onerror="window.SNAPSHOT_MISSING=true"></script>
    <style>
...
<body>
    ...
    <script>
        ...
        async function init() {
            ...
            if (!IS_OFFLINE) setInterval(loadCurriculum, 15000); 
        }
        ...
        async function fetchLesson(id, btnEl) {
            ...
            try {
                let lesson;
                if (IS_OFFLINE) {
                    lesson = SNAPSHOT_DATA.lessons[id];
                } else {
                ...
                renderLesson(lesson);
            } catch(e) { document.getElementById('lesson-content').innerText = "Error loading lesson data."; }
        }
